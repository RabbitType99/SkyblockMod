...
public class Utils {
	...
	public static void drawWaypoint(BlockPos pos, Color color, float partialTicks) {
		/**
		 * Draw a NEU like Waypoint that has a beacon beam and a outline on the
		 * target block.
		 * Credit: https://github.com/bowser0000/SkyblockMod/pull/69,
		...
		GlStateManager.disableCull();
		GlStateManager.disableTexture2D();
		System.out.println("Drew 3D Box");
		if (distSq > 5*5) renderBeaconBeam(x, y, z, color.getRGB(), 1.0f, partialTicks);
		Utils.draw3DBox(new AxisAlignedBB(pos, pos.add(1, 1, 1)), new Color(173, 216, 230).getRGB(), partialTicks);
	...
	public static void renderBeaconBeam(double x, double y, double z, int rgb, float alphaMult, float partialTicks) {
		/**
		 * Draw a beacon beam at the target location with the specified colour
		 * Credit: https://github.com/Moulberry/NotEnoughUpdates/
		 */
		int height = 300;
		int bottomOffset = 0;
		...
		tessellator.draw();
		System.out.println("Drew Beacon Beam");
	}
